...
export function CylinderInCylinder(
  ...
  if (cylinder0Min > cylinder1Max || cylinder0Max < cylinder1Min) {
    return false;
  }
...
export function BoxInSphere(
  ...
  const distX: number =
    relX > sizeX ? relX - sizeX : relX < -sizeX ? relX + sizeX : 0;
  const distY: number =
    relY > sizeY ? relY - sizeY : relY < -sizeY ? relY + sizeY : 0;
  const distZ: number =
    relZ > sizeZ ? relZ - sizeZ : relZ < -sizeZ ? relZ + sizeZ : 0;
